...
class MsRosterController extends Controller
{
    public function index($path = 'educators'){
        $details = [];
        $student = MsClassRegistration::where('year', '=', '2024-2025')->get();
        ...
        return view('backend.students.ms-roster', compact('details'))
    ...
    public function get_student($path = 'educators', $id){
        $student = MsClassRegistration::find($id);
        if(!$student)
    ...
    public function update_student(Request $req){
        ...
        if(!$req->input('id') || !$student = MsClassRegistration::find($req->input('id'))){
    ...
    public function test($path = 'educators'){
        $details = [];
        $student = MsClassRegistration::where('year', '=', '2024-2025')->get();
        foreach($student as $s){
            $id = $s->id;
        ...
        return view('backend.students.test', compact('details'))
        ->with('path', get_path($path));
    }
